...
class HistoryUnit(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.history_encoder_block1 = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=n_embedding_dim, nhead=n_hist_dec_head, dropout=dropout, activation='gelu'),
            n_hist_dec_layer, nn.LayerNorm(n_embedding_dim))
        ...
        self.snip_classifier = nn.Sequential(nn.Linear(self.history_tokens*n_embedding_dim//4, (self.history_tokens*n_embedding_dim//4)//4), 
                                            nn.ReLU(), nn.Linear((self.history_tokens*n_embedding_dim//4)//4, n_class))
    ...
    def forward(self, long_x, encoded_x, anchor_feedback=None):
        ## History Encoder
        ...
        if anchor_feedback is not None:
            if anchor_feedback.dim() == 3:
                ...
                raise ValueError(f"Unexpected anchor_feedback shape: {anchor_feedback.shape}. Expected [batch, seq_len, embed_dim]")
...
class MYNET(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=n_embedding_dim, nhead=n_enc_head, dropout=dropout, activation='gelu'),
            n_enc_layer, nn.LayerNorm(n_embedding_dim))
        
        self.decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=n_embedding_dim, nhead=n_dec_head, dropout=dropout, activation='gelu'),
        ...
        self.history_anchor_decoder_block1 = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=n_embedding_dim, nhead=n_comb_dec_head, dropout=dropout, activation='gelu'),
            n_comb_dec_layer, nn.LayerNorm(n_embedding_dim))
    ...
    def forward(self, inputs, max_iterations=2):
        ...
        ## History Driven Anchor Refinement with Feedback Loop
        decoded_anchor_feat = decoded_x
