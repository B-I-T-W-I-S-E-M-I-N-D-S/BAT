...
def train(opt): 
    ...
    for n_epoch in range(opt['epoch']):   
        ...
        print("training loss(epoch %d): %f, lr - %f" % (n_epoch, epoch_cost/(n_iter+1), optimizer.param_groups[0]["lr"]))
...
def eval_frame(opt, model, dataset, is_make_dataset=False):
    ...
    if is_make_dataset:
        return 0, 0, 0, output_cls, output_reg, {}, {}, working_time, total_frames
    else:
...
def make_dataset(opt): 
    ...
    _, _, _, output_cls, output_reg, _, _, _, _ = eval_frame(opt, model, dataset, is_make_dataset=True)
    ...
    for video_name in dataset.video_list:
        ...
        for idx in range(duration):
            ...
            for anc_idx in range(len(anchors)):
                cls = np.argwhere(cls_anc[idx, anc_idx, :-1] > threshold).reshape(-1)
                if len(cls) == 0:
                    continue
                ed = idx + anchors[anc_idx] * reg_anc[idx, anc_idx, 0]
                length = anchors[anc_idx] * np.exp(reg_anc[idx, anc_idx, 1])
                st = ed - length
                for cidx in cls:
                    proposal_anc_dict.append({
                        "segment": [st, ed],
                        "score": cls_anc[idx, anc_idx, cidx],
                        "label": cidx,
                        "gentime": idx
                    })
            all_proposals.extend(proposal_anc_dict)
        
        proposal_dict = non_max_suppression(all_proposals, overlapThresh=opt['soft_nms'])
